...
public class DragAndDropForward : MonoBehaviour, IDragHandler, IBeginDragHandler, IEndDragHandler
{
    Game game;
    bool dragging;
    Vector2 originSize;
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        for (var i = 0; i < empties.Count; i++)
        {
            ...
            if (empty.transform.position.x - 50 < this.transform.position.x  && empty.transform.position.x + 50 > this.transform.position.x
                && empty.transform.position.y - 50 < this.transform.position.y && empty.transform.position.y + 50 > this.transform.position.y)
            {
